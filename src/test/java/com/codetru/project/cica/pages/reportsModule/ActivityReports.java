...
import static com.codetru.keywords.WebUI.getElementTextsInList;
import static com.codetru.keywords.WebUI.getTextElement;
import static com.codetru.keywords.WebUI.roundOf;
import static com.codetru.keywords.WebUI.roundOfDouble;
import static com.codetru.keywords.WebUI.scrollToElementAtBottom;
import static com.codetru.keywords.WebUI.scrollToElementAtTop;
import static com.codetru.keywords.WebUI.setText;
import static com.codetru.keywords.WebUI.sleep;
import static com.codetru.keywords.WebUI.switchToMainWindow;
import static com.codetru.keywords.WebUI.validateElementContainsData;
import static com.codetru.keywords.WebUI.validateElementContainsDataAndExtractID;
import static com.codetru.keywords.WebUI.verifyContainsIgnore;
import static com.codetru.keywords.WebUI.verifyElementNotVisible;
import static com.codetru.keywords.WebUI.verifyElementVisible;
import static com.codetru.keywords.WebUI.verifyEquals;
import static com.codetru.keywords.WebUI.verifyNotContains;
import static com.codetru.keywords.WebUI.waitForPageLoaded;
...
public class ActivityReports extends CommonPageCICA { 
	...
	private By exportBtn = By
			.xpath("//span[contains(text(),'Activity Report')]/ancestor::div/descendant::ion-button[normalize-space(text())='Export']");
	...
	public void activityReports() throws Exception {
		...
		try {
			...
			DriverManager.getDriver().findElement(agencySalesSummary).isDisplayed();
			verifyElementVisible(agencySalesSummary);
			verifyElementVisible(agencyInforceReport);
			verifyElementVisible(agencyActivityReport);
		}catch (Exception e) {
			DriverManager.getDriver().findElement(independentSalesSummary).isDisplayed();
		...
		try {		
			...
			if (DriverManager.getDriver().findElement(IdNumber).isDisplayed()) {
			...
			try {
				if (DriverManager.getDriver().findElement(IdNumber).isDisplayed()) {
					Ids3 = getElementTextsInList(IdNumber);
			...
			verifyEquals(inforceNifcAppsDataDouble, ((inforcePaidAppsDataDouble + inforceMiscAppsDataDouble) - (inforceLapseAppsDataDouble + inforceNotTakenAppsDataDouble)));
			verifyEquals(inforceNifcVolumeDataDouble, ((inforcePaidVolumeDataDouble + inforceMiscVolumeDataDouble)-( inforceLapseVolumeDataDouble + inforceNotTakenVolumeDataDouble)));
			verifyEquals(inforceNifcPremiumDataDouble, roundOfDouble((inforcePaidPremiumDataDouble+ inforceMiscPremiumDataDouble) - (inforceLapsePremiumDataDouble+inforceNotTakenPremiumDataDouble)));
			clickElement(agentCaret_btn);
			clickElement(HighestLevel);
			WebUI.sleep(0.5);
			ProjectUtilities.spinnerWait(searchingSpinner);
			...
			try {
				DriverManager.getDriver().findElement(IdNumber).isDisplayed();
				...
				WebUI.verifyElementVisible(noResult);
				WebUI.sleep(0.5);
			}
