...
import static com.codetru.keywords.WebUI.getWebElements;
import static com.codetru.keywords.WebUI.mouseHoverwithaxes;
import static com.codetru.keywords.WebUI.scrollToElementAtBottom;
import static com.codetru.keywords.WebUI.selectOptionByText;
import static com.codetru.keywords.WebUI.setText;
import static com.codetru.keywords.WebUI.sleep;
import static com.codetru.keywords.WebUI.validateString;
import static com.codetru.keywords.WebUI.verifyContains;
import static com.codetru.keywords.WebUI.verifyElementClickable;
...
public class Dashboard extends CommonPageCICA {
		...
		private By ownerName=By.xpath("//tr/td[3][contains(text(), ',')]");
		...
		private By DropdownListFour=By.xpath("(//option[starts-with(text(),'7250')])[4]");
...
//			mouseHoverwithaxes(900, 520);
			...
			String extracted = policy.substring(0, 5);
			...
			clickElement(dashboard);
			setText(policyNumberField, extracted);
			clickElement(searchBtn);
			WebUI.sleep(1);
			try {
				while(DriverManager.getDriver().findElement(pleaseWaitSpinner).isDisplayed())
			...
			verifyEquals(policy_number, extracted);
			String policy_no= getTextElement(policyNumberValue);
			verifyContains(policy_no, extracted);
			clickElement(dashboard);
			...
			try {
				...
				WebUI.sleep(0.3);
				WebUI.verifyElementVisible(DropdownList);
				WebUI.clickElement(DropdownListFour);
				WebUI.sleep(1.5);
