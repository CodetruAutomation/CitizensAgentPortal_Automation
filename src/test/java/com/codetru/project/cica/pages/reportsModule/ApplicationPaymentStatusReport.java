...
public class ApplicationPaymentStatusReport extends CommonPageCICA {
	...
	private By issuedOrPendingApplicationCount = By
			.xpath("//ion-col[text()=' Total Pending/Issued Application Count ']/following-sibling::ion-col");
	private By totalAnnualPremium = By.xpath("//ion-col[text()=' Total Annual Premium ']");
	private By totalAmountAnnualPremium = By
			.xpath("//ion-col[text()=' Total Annual Premium ']/following-sibling::ion-col");
...
//	private By agentsList=By.xpath("//ion-list[@role='list']/parent::ion-list");
	...
	private By minusAmount = By.xpath(
			"(//td[starts-with(text(), 'Pending')] | //td[starts-with(text(), 'Denied')]| //td[starts-with(text(), 'Withdrawn')])/following-sibling::td[5]");
	private By issuedAppl = By.xpath(
			"(//td[starts-with(text(), 'Sub')] | //td[starts-with(text(), 'Hold')]| //td[starts-with(text(), 'Issu')]|//td[starts-with(text(), 'In')])");
	...
	public void applicationPaymentStatusReports() throws Exception {
		...
		try {
			...
			try {
				if (DriverManager.getDriver().findElement(agentNameData).isDisplayed()) {
			...
			for (int page = 1; page <= totalPagesCount; page++) {
				...
				List<String> elementTexts = elements.stream()
						...
						.filter(text -> text != null && !text.trim().isEmpty()).map(String::trim)
			...
			WebUI.scrollToElementAtBottom(exportBtn);
			WebUI.clickElement(exportBtn);
			WebUI.sleep(1);
			WebUI.switchToMainWindow();
